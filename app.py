...
with tab1:
    ...
    cover_mode = st.radio(
        "How do you want to create the cover photo?",
        ["Auto-Generate Cover Image ✨", "Upload Your Own Photo"],
        horizontal=True,
    )
    ...
    if cover_mode == "Upload Your Own Photo":
        ...
        st.markdown("<div class='step-header'>Step 1: Auto-Generate Cover Photo</div>", unsafe_allow_html=True)
        st.info("No photo needed — the system will create a cover image for you using Hugging Face AI.")
        image_prompt = st.text_input(
            "Describe the cover image (optional)",
            placeholder="A peaceful mountain lake at sunset, photorealistic",
            key="auto_prompt",
        )
    ...
    if payload_type == "Type a Message":
        ...
        st.info("💡 **Tip:** Keep it short! Auto-generated photos work best with about 5-10 seconds of audio.")
        audio_file = st.audio_input("Tap to record your secret voice message")
        if audio_file:
            data_bytes = b"AUDIO::" + audio_file.getvalue()

    button_label = "Generate Photo & Hide Secret ✨🔒" if cover_mode == "Auto-Generate Cover Image ✨" else "Hide My Secret 🔒"

    if st.button(button_label):
        if not data_bytes:
            st.warning("⚠️ Please provide a secret to hide.")
        elif cover_mode == "Upload Your Own Photo" and not cover_image:
            st.warning("⚠️ Please upload a cover photo.")
        else:
            try:
                cover_path = "temp_cover.png"

                if cover_mode == "Auto-Generate Cover Image ✨":
                    with st.spinner("Generating your cover photo with AI... 🎨"):
                        _, image_source = generate_cover_image(cover_path, prompt=image_prompt)
                    if image_source == "local_fallback":
                        st.warning(
                            "⚠️ AI image APIs are unavailable. A basic system-generated cover photo was used instead. "
                            "Add your **Pollinations API key** (`POLLINATIONS_API_KEY`) from enter.pollinations.ai to `.env`."
                        )
                    elif image_source == "pollinations":
                        st.info("🎨 Cover photo generated with **Pollinations AI** (Flux Schnell).")
                else:
                    with open(cover_path, "wb") as f:
                        f.write(cover_image.getbuffer())

                with st.spinner("Locking your secret inside the photo... 🪄"):
                    enc_data, ephemeral_key = crypto.lock_data(data_bytes)
                    stego.hide_payload(cover_path, enc_data, ephemeral_key, "secured_image.png")

                st.success("Yay! 🎉 Your secret is safely locked inside the photo.")
                st.image("secured_image.png", caption="Your secret photo", use_container_width=True)
                with open("secured_image.png", "rb") as f:
                    st.download_button(
                        "⬇️ Download Your Secret Photo",
                        data=f,
                        file_name="my_secret_photo.png",
                        mime="image/png",
                        use_container_width=True,
                    )
            except ValueError as e:
                if str(e) == "IMAGE_TOO_SMALL":
                    st.error("Oops! ❌ The secret is too big for this photo. Try a shorter recording or smaller file!")
                else:
                    st.error(f"Oops! ❌ {e}")
